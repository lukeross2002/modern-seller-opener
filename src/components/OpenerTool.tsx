...
type Stage = { afterMs: number; label: string };
...
// Loading stages are built per-generation so they can name the actual prospect
// and company — feels like the AI is doing real, custom work instead of a
// generic spinner. Driven by elapsed seconds, so labels keep rotating after
// the bar slows toward the asymptote.
function buildStages({
  ...
  const personLabel = fullName || "the prospect";
  const cleanedWebsite = companyWebsite.replace(/^https?:\/\//, "").replace(/^www\./, "").split("/")[0];
  const companyLabel = cleanedWebsite || "their company";
  return [
    { afterMs: 0,     label: `Searching the live web for ${personLabel}…` },
    { afterMs: 7000,  label: `Reading ${companyLabel}'s recent news…` },
    { afterMs: 14000, label: `Looking up ${personLabel} by name…` },
    { afterMs: 22000, label: `Verifying source dates…` },
    { afterMs: 32000, label: `Picking the sharpest trigger for ${personLabel}…` },
...
export default function OpenerTool() {
  ...
  const [stages, setStages] = useState<Stage[]>([{ afterMs: 0, label: "Starting…" }]);
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      setProgress(ratio);
      const stage = [...stages].reverse().find((s) => elapsed >= s.afterMs);
  ...
  async function handleGenerate(e: FormEvent) {
    ...
    // Lock in the stage labels for this run, naming the actual prospect/company.
  ...
  const hasResults = openers.length > 0;
  const showGate = hasResults && !unlocked;
      ...
      <div className="space-y-4">
        ...
        {!generating && hasResults && (
          <div className="relative">
            <div className={showGate ? "pointer-events-none select-none" : ""} aria-hidden={showGate}>
              <div className={showGate ? "filter blur-md" : ""}>
                {researchSummary && (
            ...
            {showGate && (
              <div className="absolute inset-0 flex items-start justify-center pt-10 md:pt-16">
                <div
                  ...
                  style={{
                    background: "linear-gradient(180deg, rgba(12,12,20,0.96) 0%, rgba(12,12,20,0.98) 100%)",
                    boxShadow: "0 30px 80px -20px rgba(141,123,255,0.35), 0 0 0 1px rgba(141,123,255,0.25)",
                  }}
                >
                  <p className="eyebrow mb-3">Your openers are ready</p>
                  <h3 className="text-2xl font-bold tracking-[-0.02em] leading-tight">
                    Don&apos;t lose these openers — <span className="gradient-text">where should we send them?</span>
                  </h3>
                  <p className="mt-3 text-[color:var(--muted-soft)] leading-relaxed text-[15px]">
                    Drop your name and email. We&apos;ll unlock all 3 right now and send a copy to your inbox.
                  </p>
                  <form onSubmit={handleUnlock} className="mt-5 space-y-3">
                    ...
                    <button type="submit" className="btn-primary w-full justify-center" disabled={unlockLoading}>
                      {unlockLoading ? "Unlocking…" : "Unlock my openers"} <span aria-hidden>→</span>
                    </button>
                  </form>
                </div>
              </div>
        ...
        {!generating && hasResults && unlocked && (
